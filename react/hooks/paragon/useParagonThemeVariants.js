      ...
      var _createThemeVariantLink = function createThemeVariantLink(url) {
        ...
        themeVariantLink.onerror = function () {
          ...
          if (isFallbackThemeUrl) {
            (0, _logging.logError)("Could not load theme variant (".concat(paragonThemeAccessor, " - ").concat(themeVariant, ") CSS from fallback URL. Aborting."));
            if (isBrandOverride) {
              setIsBrandThemeVariantComplete(true);
            } else {
              setIsParagonThemeVariantComplete(true);
            ...
            (0, _utils.removeExistingLinks)(otherExistingLinks);
            return;
          }
          ...
          if (variants[themeVariant]) {
            var themeVariantFallbackUrl = (0, _utils.fallbackThemeUrl)(variants[themeVariant].fileName);
            (0, _logging.logInfo)("Failed to load theme variant (".concat(themeVariant, ") CSS from ").concat(isBrandOverride ? value.urls.brandOverride : value.urls["default"], ". Falling back to locally installed theme variant: ").concat(themeVariantFallbackUrl));
            ...
            (0, _utils.removeExistingLinks)(_otherExistingLinks);
            ...
            (0, _logging.logError)("Failed to load theme variant (".concat(themeVariant, ") CSS from ").concat(url, " and locally installed fallback URL is not available. Aborting."));
            if (isBrandOverride) {
              setIsBrandThemeVariantComplete(true);
            } else {
              setIsParagonThemeVariantComplete(true);
...
var _default = exports["default"] = useParagonThemeVariants;
